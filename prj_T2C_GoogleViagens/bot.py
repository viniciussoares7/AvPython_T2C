...
class Bot(WebBot):
    ...
    def action(self, execution=None):
        ...
        var_strQuery = "SELECT * FROM tbl_Dados_Passagem order by valor asc"

        con = sqlite3.connect(var_strPathToDb).execute(var_strQuery)

        var_dfDadosSelect = pd.DataFrame(con)

        con.close()

        #cria excel e insere os dados do processo
        caminho_do_arquivo = r'C:\Users\vinicius.soares\OneDrive - T2C CONSULTORIA LTDA\Documentos\avaliacoes T2C\Python\Saidas\passagens.xlsx'

        with pd.ExcelWriter(caminho_do_arquivo, engine='openpyxl') as writer:
            var_dfDadosSelect.head(10).to_excel(writer, sheet_name='Baratos', index=False)
            var_dfDadosSelect.to_excel(writer, sheet_name='Todos', index=False)
        ...
        if(var_dictConfig["EmailFinal"].upper() == "SIM"):   
            ...
            var_clssSendEmailOutlook.send_email_final(arg_strHorarioInicio=var_strDatahoraInicio, arg_strHorarioFim=var_strDatahoraFim, arg_strEnvioPara=var_dictConfig["EmailDestinatarios"], arg_listAnexos=[var_clssRelatorios.var_strPathRelatorioAnalitico, var_clssRelatorios.var_strPathRelatorioSintetico,caminho_do_arquivo], arg_boolSucesso=True)
